...
#### `electronWrapPackaged` {#electron-wrap-hook-is-packaged}

Controls whether the Electron pacakged is told it is in a production package.
This variable is defauled to `true`, so if the attribute is not set, it is assumed that
...
```nix
{
  electronWrapPackaged = false;
}
```

The reason that some application break under this is because it changes how Electron passes the elements of `process.argv` to the application.

When `ELECTRON_FORCE_IS_PACKAGED` is false, Electron runs in "dev mode", meaning that arvg looks like:
```js
[
  "/nix/store/aaaaaaaaaaaaaaaaaaaaaaaaaaaaa-electron/bin/electron",
  "/nix/store/aaaaaaaaaaaaaaaaaaaaaaaaaaaaa1-coolapp/share/coolapp/resources/app.asar",
  "--version"
]
```
Notice that the second argument is the path to the `asar`. When the `ELECTRON_FORCE_IS_PACKAGED` is true, it is the following:
```js
[
  "/nix/store/aaaaaaaaaaaaaaaaaaaaaaaaaaaaa-electron/bin/electron",
  "--version"
...
```
In "production mode," Electron drop the `asar` path. Not all apps handle this correctly. The proper way to handle this is the [following code](https://github.com/electron/electron/issues/4690#issuecomment-422617581):
```js
if (app.isPackaged) {
